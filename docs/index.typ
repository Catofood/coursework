...
=== REST API endpoints 
StatisticsController предоставляет методы с базовым URL `/api/statistics`. Описание функциональности каждого эндпоинта представлено ниже с соответствующими примерами запросов и ответов.
...
#figure(
  ...
  caption: [Результаты выполнения автоматических тестов],
  kind: image
) <fig-test-results>
